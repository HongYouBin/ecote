...
- 벽을 세우기 위해 조합을 이용했다. 전체 경우의 수가 조합 36_C_3이라 시간을 초과하지 않는다.
- 초반 문제를 구현하는데 flag 배열의 수를 잘못 설정하여 애를 먹었다. flag 배열을 선생 수와 같게 설정해야 하는데, 실수로 3개를 설정했다. 실수하지 않도록 주의해야 한다.
- 처음 flag를 배열로 설정하여 풀었는데 너무 복잡했다. 단순화하여 가독성을 높이기 위해 flag를 배열이 아닌 하나의 변수로 설정하여 조건에 맞지 않으면 bool 변수(flag)를 true로 조건에 맞으면 flase로 설정 후 74행부터 답을 출력하도록 했다.
- 선생이 학생을 만는지 안 만나는지 재귀로 구현했다. 답은 반복문을 사용했다.
- 선생과 학생이 만나는 여부를 확인하는 조건문이 4개가 나왔다. 코드가 비슷하여 복붙했다가 return에서 복사한 내용을 수정하지 않는 실수를 했다.
...
https://github.com/HongYouBin/ecote/blob/main/46.cpp

https://www.acmicpc.net/problem/16236

- n이 최대 20밖에 안되기 때문에 구현문제인 것을 바로 알았다. 처음에 물고기 개수를 기준으로 정리된 2차원 vector를 만들어볼까 생각했다. ex)vt[1] -> 물고기 칸이 1인 좌표. 이런 방식으로 설계를 해보니 비효율적이어서 그냥 bfs 알고리즘을 사용하는게 최선이라는 생각이 들었다.
- bfs로 구현했다. 물고기를 먹기 위해 bfs 연산을 하고 먹을 수 있는 가장 가까운 물고기의 좌표를 구하는 방식으로 구현했다.
- 아이디어는 쉽게 떠올랐지만 구현하기 까다로운 조건이 많아 어려웠다. 지문을 정확하게 읽고 구현해야 하는 까다로운 조건을 메모한 뒤 빼먹지 않았나 확인하는 방법으로 실수를 줄여야 될 것 같다.
  - 23행) 처음 상어가 시작하는 지점(배열을 받을 때 9인 지점)을 0으로 바꿔주는 연산을 해야 하지만 실수로 하지 않았다.
  - 61행) 상어가 물고기를 먹으면 사이즈를 키워줘야 되는데 실수로 빼먹었다.

### 47번 청소년 상어 404p

https://github.com/HongYouBin/ecote/blob/main/47.cpp

https://www.acmicpc.net/problem/19236

- 기능에 따라 함수를 나눠서 풀으니 가독성이 높아졌다. 문제가 복잡해보일수록 설계과정을 철저하게 해야한다. 설계 과정에서 조건들을 확인하고 함수에 따라 기능을 분리하는게 좋다. 어려운 문제일수록 설계 필수!!!
- 배열의 총 개수가 4\*4개여서 완전탐색으로 문제를 풀었다. dfs 알고리즘으로 모든 경우를 탐색했다.
- 가장 까다로웠던 부분은 물고기를 이동하는 과정과, 물고기를 원래 위치로 되돌리는 과정을 반복해야 했다. 2차원 배열을 매개 변수로 넘겨서(40행, 11행) 배열을 복사하는 과정을 거쳤다.(46행) 2차원 배열을 매개변수로 넘기는 과정에서 배열 포인터를 사용했다.
- 46행에서 3중 for문을 사용하는 실수를 했다. 배열을 복사만 하면 되기 때문에 2중 for문만 사용하면 된다. 아마도 상어의 위치를 4번 움직여야 한다는 생각때문에 3중 for문을 사용한 것 같다. 미리 설계를 안해서 이런 실수가 발생했다. 어려운 문제일수록 설계를 정확하게 해야한다.
- 문제를 다 풀어놓고 답이 안나왔는데 58행에서 실수했다. 상어의 위치를 움직이지 않고, 같은 위치를 탐색하도록 실수했다.
  - 수정전 58행 : nRow = row + dx[dir-1]

### 48번 어른 상어 407p

https://github.com/HongYouBin/ecote/blob/main/48.cpp

https://www.acmicpc.net/problem/19237

- 문제를 읽고 바로 코딩하지 않고, 노트에 설계를 하고 구현했다. 덕분에 변수 할당, 처리해야 하는 조건, 구현해야 하는 함수, 함수의 순서 설계등을 쉽게 할 수 있었다. 문제를 보고 바로 코딩하지 않고, 먼저 노트에 어떻게 할지 설계한 후 작성하는 습관을 길러야 한다.
- 상어 방향, 좌표, 냄새 좌표, 상어의 방향 순서등을 한번에 놓지 않고 모두 따로 배열로 구현했다.
- 상어의 다음 위치를 고르는 과정 설계가 가장 까다로웠다.상어가 갈 수 있는 모든 방향을 구한 뒤 우선 순위가 높은 방향을 선택하도록 했고, 답은 맞췄지만 코드가 깔끔하게 나오진 않았다. 답을 보니 우선 순위가 높은 방향을 먼저 탐색한 후 갈 수 있는지 여부를 그 후에 확인했다. 가독성, 시간 절약 부분에서 답이 좋아보였다. 다음 회차에 풀 때 사용해볼 예정이다.
- 상어의 위치를 2차원 배열이 아닌 vector로 x, y좌표를 받아 저장했다. 나의 풀이와 달리 답에서는 2차원 배열로 저장했다. 내 풀이는 vector로 상어의 위치를 받았기 때문에 상어가 죽을 때 1차원 배열(dead)을 만들어 상어가 죽었다는 표시를 해야 했다. 상어를 다 옮긴 후 chk 함수(14행)을 만들어 상어가 겹친 상태인지 아닌지 확인해줬다. 답에서는 2차원 배열에서 상어가 겹칠 때 다른 배열 필요 없이 작은 값이 배열에 들어가도록 설정하면 된다. 따라서 움직인 후 겹쳤는지 확인할 필요가 없어 나의 풀이보다 시간을 절약했다. 상어들이 겹치는 경우는 상어 각각 냄새가 빈 공간으로 갈 때만 겹친다. 한 상어라도 자신의 냄새가 나는 곳으로 가면 상어가 겹치는 경우가 발생하지 않는다.
- 조건들의 순서 특히 시간 증가를 어떻게 해야하나 고민했다. 나의 풀이는 1.냄새 감소 2.이동 3.냄새할당 4.시간증가 5.종료조건확인 답은 1.냄새감소 2.냄새할당 3.이동 4.시간증가 5.종료조건확인 순서로 맞췄다. 시뮬레이션 문제이기 때문에 최대한 문제 과정과 비슷하게 순서를 맞춰야한다. 비슷한 문제가 나오면 시간 증가와 종료조건을 마지막에 두는게 좋아보인다.
